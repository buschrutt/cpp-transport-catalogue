...
namespace json {
    ...
    bool Node::IsDict() const {if(std::holds_alternative<Dict>(value_)){return true;} else {return false;}}
    ...
    void JsonOutput (const Document& doc, std::ostream& output) {
        ...
        if (doc.GetRoot().IsDict()){
            ...
            output << "{"s;
            for (auto [key, value] : doc.GetRoot().AsDict()){
                if (is_first_pair){
                    is_first_pair = false;
                } else {
                    output << ", "s;
    ...
    bool operator== (const Node & l, const Node & r){
        ...
        if(l.IsDict() && r.IsDict()){if(l.AsDict() == r.AsDict()){return true;}}
        return false;
    }

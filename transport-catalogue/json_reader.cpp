            ...
            for (const auto& db_request : db_request_arr.AsArray()){
                ...
                if(node.AsString() == "Bus"s){
                    ...
                    catalogue.SetBus(db_request.AsMap().at("name"s).AsString()
                                     , stop_data
                                     , !db_request.AsMap().at("is_roundtrip"s).AsBool());
                } else {
                    geo::Coordinates coordinates{};
                    coordinates.lat = db_request.AsMap().at("latitude"s).AsDouble();
                    coordinates.lng = db_request.AsMap().at("longitude"s).AsDouble();
                ...
                if(node.AsString() == "Bus"s){
                    ...
                    if (catalogue.BusStopCount(bus_name) == 0){
                        ...
                        json_arr.emplace_back(json_lib::Dict{
                                ...
                                {"route_length"s, catalogue.BusRouteLength(bus_name).first},
                                {"curvature"s, catalogue.BusRouteLength(bus_name).second}
                        });
                    }
                } else {
